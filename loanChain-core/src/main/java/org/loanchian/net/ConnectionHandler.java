...
import static org.loanchian.utils.Utils.checkNotNull;
import static org.loanchian.utils.Utils.checkState;
...
class ConnectionHandler implements MessageWriteTarget {
    ...
    public ConnectionHandler(StreamConnection connection, SelectionKey key, SocketChannel channel, Set<ConnectionHandler> connectedHandlers) {
    	this.key = key;
    ...
    public void writeBytes(byte[] message) throws IOException {
        ...
        try {
            ...
            lock.unlock();
            andUnlock = false;
            log.warn("Error writing message to connection, closing connection {}", e.getMessage());
            closeConnection();
